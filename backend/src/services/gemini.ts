...
Respond strictly in JSON:
{
  ...
  public async explainBioequivalency(promptText: string): Promise<string> {
    if (!this.aiClient) {
      return 'Therapeutically bioequivalent FDA Orange Book approved generic equivalent formulation.';
    }
    try {
      const response = await this.aiClient.models.generateContent({
        model: 'gemini-2.5-flash',
      ...
      return response.text || 'Bioequivalent generic equivalent verified.';
    } catch {
      return 'Bioequivalent formulation matching active chemical identity.';
    }
  }
}
