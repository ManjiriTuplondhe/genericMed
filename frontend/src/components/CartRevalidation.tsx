...
import { CartItem, PaymentIntent } from '../types';
...
export const CartRevalidation: React.FC<CartRevalidationProps> = ({
  ...
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState<PaymentIntent['paymentMethod']>('card');
  const [isProcessing, setIsProcessing] = useState<boolean>(false);
  const [isSuccessModalOpen, setIsSuccessModalOpen] = useState<boolean>(false);
  const [confirmedOrderId, setConfirmedOrderId] = useState<string>('GM-88241');
  const [paymentResult, setPaymentResult] = useState<PaymentIntent | null>(null);
  ...
  const handlePayClick = async () => {
    ...
    try {
      ...
      setPaymentResult(payRes.payment);
  ...
  return (
    <div className="max-w-md mx-auto bg-[#faf8ff] text-[#131b2e] min-h-screen relative pb-32 shadow-xl border-x border-[#dae2fd]/40">
      ...
      <main className="px-4 py-3 flex flex-col gap-3.5">
        ...
        {/* Payment & Escrow Authorization Card */}
        <section className="bg-white rounded-xl p-3.5 shadow-sm border border-[#eaedff] flex flex-col gap-2.5">
          <div className="flex items-center justify-between">
            <span className="font-headline font-bold text-[13px] text-[#131b2e]">
              Payment Method & Escrow Protection
            </span>
            <div className="flex items-center gap-1 text-[10px] text-[#006947] font-bold bg-[#6ffbbe]/20 px-2 py-0.5 rounded-full">
              <span className="material-symbols-outlined text-[13px]">shield</span>
              <span>Escrow Held Until Handover</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              ...
              className={`p-2 rounded-lg border text-center transition-all ${
                selectedPaymentMethod === 'card'
                  ? 'border-[#00685f] bg-[#00685f]/10 text-[#00685f] font-bold'
                  : 'border-gray-200 text-gray-700 hover:bg-gray-50'
              }`}
            >
              <div className="text-xs">Credit Card</div>
              <div className="text-[10px] text-gray-500">•••• 4242</div>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <div className="text-[10px] text-gray-500">1-Touch Pay</div>
            </button>
            <button
              type="button"
              onClick={() => setSelectedPaymentMethod('fsa_hsa')}
              className={`p-2 rounded-lg border text-center transition-all ${
                selectedPaymentMethod === 'fsa_hsa'
                  ? 'border-[#00685f] bg-[#00685f]/10 text-[#00685f] font-bold'
                  : 'border-gray-200 text-gray-700 hover:bg-gray-50'
              }`}
            >
              <div className="text-xs">FSA / HSA</div>
              <div className="text-[10px] text-emerald-600 font-semibold">100% Eligible</div>
            </button>
          </div>
          ...
          <div className="bg-[#f2f3ff] p-2.5 rounded-lg flex items-center justify-between border border-[#eaedff]">
            <div className="flex items-center gap-2">
              <div className="w-8 h-6 bg-[#131b2e] text-white rounded text-[10px] font-bold flex items-center justify-center tracking-tighter">
                {selectedPaymentMethod === 'apple_pay' ? 'Pay' : selectedPaymentMethod === 'fsa_hsa' ? 'HSA' : 'MC'}
              </div>
              <div className="flex flex-col">
                <span className="font-label-sm text-[12px] font-bold text-[#131b2e]">
                  {selectedPaymentMethod === 'apple_pay' ? 'Apple Pay (Quick Checkout)' : selectedPaymentMethod === 'fsa_hsa' ? 'Health Savings Debit Card' : 'Mastercard •••• 4242'}
                </span>
                <span className="font-body-sm text-[10px] text-[#6d7a77]">
                  Funds held in escrow • Pharmacist paid upon cleanroom OTP match
                </span>
              </div>
            </div>
          ...
          >
            {isProcessing ? (
              <>
                ...
                <span>Locking Batch & Escrow...</span>
              </>
            ) : (
              <>
                <span className="material-symbols-outlined text-[18px]">verified</span>
                <span>Lock & Hold ${networkTotal.toFixed(2)} in Escrow</span>
              </>
            )}
          </button>
        </div>
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-gray-100 flex flex-col items-center text-center gap-3 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="bg-[#f2f3ff] rounded-xl p-3 w-full text-left font-body-sm text-[12px] flex flex-col gap-1 border border-[#eaedff]">
              ...
              {paymentResult && (
                <div className="flex justify-between pt-1 border-t border-gray-200 text-[11px]">
                  <span className="text-gray-600">Payment Escrow ID:</span>
                  <span className="font-mono text-gray-700">{paymentResult.id}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2 w-full mt-2">
              <button
                ...
                className="w-full py-2.5 bg-[#00685f] text-white rounded-xl font-semibold text-xs hover:bg-[#008378] transition-colors flex items-center justify-center gap-1.5"
              >
                <span className="material-symbols-outlined text-[16px]">local_shipping</span>
                <span>Track Live Order & View Receipt</span>
              </button>
              <button
                onClick={onBack}
