...
import { Medicine, UserProfile, PrescriptionRecord } from '../types';
...
export const CustomerHome: React.FC<CustomerHomeProps> = ({
  ...
  const [selectedMedForInsurance, setSelectedMedForInsurance] = useState<Medicine | undefined>(undefined);
  ...
  const handlePrescriptionParsed = (record: PrescriptionRecord) => {
    // Navigate to matching drug
    const matched = medicines.find(
      (m) =>
        m.name.toLowerCase().includes(record.genericMatched.toLowerCase()) ||
        m.genericName.toLowerCase().includes(record.genericMatched.toLowerCase())
    );
    if (matched) {
      onNavigateToDetail(matched.id);
    }
  };
    ...
    <div className="max-w-md mx-auto bg-[#faf8ff] text-[#131b2e] min-h-screen relative pb-28 shadow-xl border-x border-[#dae2fd]/40">
        ...
        <div className="px-4 py-2 flex flex-col gap-1">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button
                aria-label="Insurance Copay Calculator"
                onClick={() => {
                  setSelectedMedForInsurance(medicines[0]);
                  setIsInsuranceModalOpen(true);
                }}
                className="h-10 px-2.5 flex items-center gap-1 rounded-xl bg-primary/10 text-primary hover:bg-primary/20 transition-colors text-xs font-bold"
                title="Insurance vs Cash Copay Calculator"
              >
                <span className="material-symbols-outlined text-[18px]">calculate</span>
                <span className="hidden sm:inline">Copay Calc</span>
              </button>
              <button
                aria-label="Cart"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col min-w-0 flex-1">
              ...
              <p className="font-body-sm text-[12px] text-[#3d4947] line-clamp-2 mt-0.5">
                Snap a photo and our Gemini Vision OCR matches the cheapest approved generics across local pharmacies instantly.
              </p>
            </div>
          </div>
          <button
              ...
              >
                ...
                <div className="flex items-center justify-between pt-0.5">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={() => {
                        setSelectedMedForInsurance(med);
                        setIsInsuranceModalOpen(true);
                      }}
                      className="px-2.5 py-1.5 bg-[#f2f3ff] hover:bg-[#e2e7ff] text-[#00685f] rounded-lg font-label-md text-[11px] font-semibold flex items-center gap-1 border border-[#eaedff] transition-colors"
                      title="Calculate Insurance Copay"
                    >
      ...
      <nav className="fixed bottom-0 max-w-md w-full z-40 bg-[#faf8ff]/95 backdrop-blur-xl shadow-[0_-1px_8px_rgba(0,0,0,0.06)] border-t border-[#eaedff]">
        <div className="flex justify-around items-center h-16 px-1">
          ...
          <button
            onClick={() => setIsPrescriptionModalOpen(true)}
            className="flex flex-col items-center justify-center gap-0.5 min-w-[64px] py-1 text-[#6d7a77] hover:text-[#00685f] transition-colors"
          >
            <span className="material-symbols-outlined text-[22px]">document_scanner</span>
            <span className="font-label-sm text-[10px]">Scan Rx</span>
          </button>
          <button
            onClick={onNavigateToOrders || onNavigateToCart}
      ...
      {/* Insurance Copay Calculator Modal */}
      <InsuranceCalculatorModal
        isOpen={isInsuranceModalOpen}
        onClose={() => setIsInsuranceModalOpen(false)}
        medicine={selectedMedForInsurance}
      />
    </div>
  );
};
